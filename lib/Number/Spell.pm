...
require Exporter;
## modified by harshal 30th october 2009
...
sub spell_number{
  ...
  if(defined($opts{Format})&&($opts{Format} eq "in")){
      #Indian formatting
      my $c=1;
	if ($l <= 5 || $l > 9) {
		$ret = spell_number($data, { Format => 'eu' }) 
	}else {
		while( length($d) <= 9) {
			$d = "0" . $d;
		}		
		...
		$d =~ /(..)(..)(.....)$/;
		my ($cr , $lc, $th) = ( $1, $2, $3);
		$cr = spell_number($cr, { Format => 'eu' }) if int($cr);
		$lc = spell_number($lc, { Format => 'eu' }) if int($lc);
		$th = spell_number($th, { Format => 'eu' });
		
		$ret = "$lc Lac $th";
		$ret = "$cr Crore " . $ret if $cr ne "00";
	}
    
    $ret .= " and " . spell_number($dec, { Format => 'eu' }) if int($dec);
    
    }elsif(defined($opts{Format})&&($opts{Format} eq "eu")){
      #European formatting
